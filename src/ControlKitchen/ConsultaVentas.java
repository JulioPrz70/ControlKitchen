...
public class ConsultaVentas extends javax.swing.JFrame {
    ...
    public ConsultaVentas() {
        ...
        noVenta(); //MUESTRA EL ULTIMO ID DE LA VENTA
    ...
    private void initComponents() {
        ...
        TablaConsulta.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Id_Venta", "Fecha", "Hora", "Estado", "Monto_Total"
            }
        ) {
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "BUSCAR VENTA", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Times New Roman", 1, 13))); // NOI18N
    ...
    private void txtIdVentaKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtIdVentaKeyPressed
        ...
        String[] titulos = {"Id_Venta", "Fecha_Venta", "Hora_Venta", "Estado", "Monto_Total"};
        String[] registros = new String[200];
        //SE CONSULTA LOS DATOS CON BASE A LO QUE SE INSERTA EN EL CAMPO ID O ESTADO
        String sql = "SELECT * FROM gestionventa WHERE Id_Venta LIKE '" + txtIdVenta.getText() + "%'";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                registros[2] = rs.getString("Hora_Venta");
    ...
    private void btnBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBuscarActionPerformed
        ...
        int bus = Integer.parseInt(txtIdVenta.getText());
        int max = Integer.parseInt(txtMax.getText());
        if (bus < max) { //SE EVALUA SI NO SE HACE UNA BUSQUEDA SUPEIOR A LA EXISTENTE
            gestionventa(); //MUESTRA EN LA TABLA DE LA VENTANA RESULTADOVENTAS LA GESTIONVENTA
            detalleventa(); //MUESTRA EN LA TABLA DE LA VENTANA RESULTADOVENTAS LOS DETALLES DE LA VENTA
        } else {
            JOptionPane.showMessageDialog(null, "El Id_Venta Ingresado Es Mayor A La Existente.");
        }
    }//GEN-LAST:event_btnBuscarActionPerformed

    //MANDA LOS DATOS DE GESTION VENTA A LA OTRA VENTANA CON BASE AL ID
    private void gestionventa() {
        String[] titulos = {"Id_Venta", "Cliente", "Usuario", "Fecha_Venta", "Hora_Venta", "Estado", "Monto_Total"};
        String[] registros = new String[200];
        //CONSULTA LOS DATOS PARA MOSTRAR
        String sql = "SELECT Id_Venta, gestioncliente.Nombre, Username, Fecha_Venta, Hora_Venta, Estado, Monto_Total FROM gestionventa INNER JOIN gestioncliente USING(Id_Cliente) INNER JOIN gestionusuarios USING(Id_Usuario) WHERE Id_Venta = '" + txtIdVenta.getText() + "'";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                registros[4] = rs.getString("Hora_Venta");
                registros[5] = rs.getString("Estado");
                registros[6] = rs.getString("Monto_Total");
                model.addRow(registros); //SE AGREGA A UNA NUEVA FILA
            }
            //SE ABRE LA VENTANA RESULTADO VENTAS PARA MOSTRAR RESULTADOS DE LA CONSULTA ANTERIOR
            ResultadoVentas L = new ResultadoVentas();
            ResultadoVentas.TablaVenta.setModel(model); //SE CARGA EN LA TABLA DE LA VENTANA RESULTADO VENTAS LOS RESULTADOS.
    ...
    private void detalleventa() {
        ...
        String[] titulos = {"Id_DetalleVenta", "Id_Venta", "Producto", "Cantidad", "Costo_Unitario", "SubTotal"};
        String[] registros = new String[200];
        //CONSULTA DE LOS DATOS QUE SERAN MOSTRADOS EN LA TABLA
        String sql = "SELECT Id_DetalleVenta, Id_Venta, Nombre, Cantidad, Costo_Unitario, SubTotal FROM detalleventa INNER JOIN gestionproducto USING(Id_Producto) WHERE Id_Venta = '" + txtIdVenta.getText() + "'";
        ...
        try {
            ...
            //SE CARGA LOS DATOS EN EL ARREGLO REGISTROS Y SE MANDA AL MODEL PARA MOSTRAR
            while (rs.next()) {
                registros[0] = rs.getString("Id_DetalleVenta");
            ...
            ResultadoVentas.TablaDetalle.setModel(model); //SE CARGA LOS RESULTADOS EN LA TABLA DE LA VENTANA RESULTADO VENTAS
        } catch (SQLException e) {
            System.out.println("ERROR AL BUSCAR LOS DATOS " + e.getMessage());
        }
    }
    ...
    private void cargarTabla() {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT Id_Venta, Fecha_Venta, Hora_Venta, Estado, Monto_Total FROM gestionventa");
            rs = ps.executeQuery();
            rsmd = rs.getMetaData();
            ...
            while (rs.next()) {
                ...
                modeloTabla.addRow(fila); //SE AGREGA EN UNA NUEVA FILA POR CADA VENTA
            }
        } catch (SQLException e) {
    ...
    //ARROJA EL ULTIMO NO. DE LA VENTA PARA PODER PONER EL LIMITE DE BUSQUEDA
